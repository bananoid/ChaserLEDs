...
#define DEBUG_PIN_RED 33
#define DEBUG_PIN_GREEN 34

void setup()
{
  Serial.begin(9600); //Open serial connection at 9600 baud

  pinMode(DEBUG_PIN_RED, OUTPUT);
  pinMode(DEBUG_PIN_GREEN, OUTPUT);
...
word requestRange()
{
  ...
  {                                       //Sensor responded with the two bytes
    ...
    // analogWrite(DEBUG_PIN_RED, (262.2857 - 0.3643 * range));
    // analogWrite(DEBUG_PIN_RED, (262.2857 - 0.3643 * range));

    float ledI = 255.0 - range * 2.0;
    ledI = max(0, ledI);
    ledI = min(255, ledI);
    analogWrite(DEBUG_PIN_RED, ledI);
